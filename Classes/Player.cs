...
namespace Classes
{
    public struct StatValue : IDataIO
    {
        ...
        public StatValue(int[, ,] _raceSexMinMax, int[] _classMin, int[] _ageEffects, int _cur_offset = 0, int _full_offset = 0, int _min = 3, int _max = 25)
        {
    ...
    {
        //public bool[] spellBook = new bool[gbl.spellCastingTable.Length];
        ...
        void IDataIO.Write(byte[] data, int offset)
        {
            foreach (SpellEntry spell in gbl.spellCastingTable)
            {
                ...
                {
                    if (spell.spellClass == SpellClass.MagicUser)
                    {
                        data[offset + spell.spellIdx - 1] = (byte)(spellBook[spell.spellIdx] == true ? 1 : 0);
                    }
                    else
                    {
                        data[offset + spell.spellIdx - 1] = 0;
                    }
                }
            }
        }
