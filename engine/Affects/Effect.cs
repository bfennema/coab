...
namespace engine.Affects
{
    ...
    {
        delegate Task<bool> affectDelegate(Classes.Effect arg_0, object affect, Classes.Player player);
        ...
        internal static async Task<bool> Call(Classes.Effect add_remove, object parameter, Classes.Player player, Classes.Affects affect) /* sub_630C7 */
        {
            ...
            {
                return await func(add_remove, parameter, player);
            }

            return false;
        }
        static internal async Task<bool> Check(Classes.Player player, CheckType type) // work_on_00
        {
            switch (type)
            {
                case CheckType.None:
                ...
                case CheckType.Visibility:
                    await ovr024.calc_affect_effect(Classes.Affects.blink, player);
                    await ovr024.calc_affect_effect(Classes.Affects.invisibility, player);
                    await ovr024.calc_affect_effect(Classes.Affects.invisible, player);
                    await ovr024.calc_affect_effect(Classes.Affects.invisible_to_animals, player);
                    break;

                case CheckType.PostHit1_Damage:
                    await ovr024.calc_affect_effect(Classes.Affects.fireAttack_2d10, player);
                    await ovr024.calc_affect_effect(Classes.Affects.ankheg_melee_acid_attack, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dispel_evil_banish, player);
                    await ovr024.calc_affect_effect(Classes.Affects.engulf, player);
                    await ovr024.calc_affect_effect(Classes.Affects.owlbear_hug_check, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dracolich_paralysis, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dracolich_cold_damage, player);
                    break;

                case CheckType.PostHit2_Damage:
                    await ovr024.calc_affect_effect(Classes.Affects.poison_plus_0, player);
                    await ovr024.calc_affect_effect(Classes.Affects.poison_plus_4, player);
                    await ovr024.calc_affect_effect(Classes.Affects.poison_plus_2, player);
                    await ovr024.calc_affect_effect(Classes.Affects.thri_kreen_paralyze, player);
                    await ovr024.calc_affect_effect(Classes.Affects.poison_neg_2, player);
                    await ovr024.calc_affect_effect(Classes.Affects.fireAttack_2d10, player);
                    await ovr024.calc_affect_effect(Classes.Affects.beholder_eyestalk, player);
                    break;

                case CheckType.SpecialAttacks:
                    await ovr024.calc_affect_effect(Classes.Affects.ray_of_enfeeblement, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_flame_tongue, player);
                    await ovr024.calc_affect_effect(Classes.Affects.salamander_heat_damage, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_dragon_slayer, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_frost_brand, player);
                    await ovr024.calc_affect_effect(Classes.Affects.ranger_vs_giant, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_undead_slayer, player);
                    break;

                case CheckType.Type_5:
                    await ovr024.calc_affect_effect(Classes.Affects.mirror_image, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_from_normal_missiles, player);
                    await ovr024.calc_affect_effect(Classes.Affects.thri_kreen_dodge_missile, player);
                    await ovr024.calc_affect_effect(Classes.Affects.boulder_evasion, player);
                    await ovr024.calc_affect_effect(Classes.Affects.troll_regen, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_pierce_slash, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_magic_weapon, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protect_non_magic_weapons, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_blunt_pierce, player);
                    await ovr024.calc_affect_effect(Classes.Affects.vuln_holy_water, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_normal_weapons, player);
                    await ovr024.calc_affect_effect(Classes.Affects.half_damage, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_fire_and_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_piercing, player);
                    await ovr024.calc_affect_effect(Classes.Affects.vuln_blessed_quarrel, player);
                    await ovr024.calc_affect_effect(Classes.Affects.fire_shield_damage, player);
                    break;

                case CheckType.PreDamage:
                    await ovr024.calc_affect_effect(Classes.Affects.efreeti_fire_resist, player);
                    await ovr024.calc_affect_effect(Classes.Affects.item_fire_resist, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_frost_brand, player);
                    await ovr024.calc_affect_effect(Classes.Affects.spell_resist_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.spell_resist_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_magic_50_percent, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_magic_15_percent, player);
                    await ovr024.calc_affect_effect(Classes.Affects.immune_to_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.half_elec, player);
                    await ovr024.calc_affect_effect(Classes.Affects.half_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.shield, player);
                    await ovr024.calc_affect_effect(Classes.Affects.half_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.troll_regen, player);
                    await ovr024.calc_affect_effect(Classes.Affects.mirror_image, player);
                    await ovr024.calc_affect_effect(Classes.Affects.immune_to_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_drag_breath, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_fire_and_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.shambling_absorb_lightning, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protect_magic, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dracolich_protection, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protect_elec, player);
                    await ovr024.calc_affect_effect(Classes.Affects.minor_globe_of_invulnerability, player);
                    break;

                case CheckType.PlayerRestrained:
                    await ovr024.calc_affect_effect(Classes.Affects.snake_charm, player);
                    await ovr024.calc_affect_effect(Classes.Affects.paralyze, player);
                    await ovr024.calc_affect_effect(Classes.Affects.sleep, player);
                    await ovr024.calc_affect_effect(Classes.Affects.helpless, player);
                    await ovr024.calc_affect_effect(Classes.Affects.sticks_to_snakes, player);
                    await ovr024.calc_affect_effect(Classes.Affects.fumbling, player);
                    await ovr024.calc_affect_effect(Classes.Affects.entangle, player);
                    break;

                case CheckType.BattleSetup1:
                    await ovr024.calc_affect_effect(Classes.Affects.fight_unconscious, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_fire_and_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.displace, player);
                    await ovr024.calc_affect_effect(Classes.Affects.camouflage, player);
                    await ovr024.calc_affect_effect(Classes.Affects.item_invisibility, player);
                    break;

                case CheckType.MagicResistance:
                    await ovr024.calc_affect_effect(Classes.Affects.resist_magic_50_percent, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_magic_15_percent, player);
                    await ovr024.calc_affect_effect(Classes.Affects.elf_resist_sleep, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protect_charm_sleep, player);
                    await ovr024.calc_affect_effect(Classes.Affects.resist_paralyze, player);
                    await ovr024.calc_affect_effect(Classes.Affects.immune_to_cold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_paralysis_poison, player);
                    await ovr024.calc_affect_effect(Classes.Affects.immune_to_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.halfelf_resistance, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_sleep_charm_paralysis_poison, player);
                    await ovr024.calc_affect_effect(Classes.Affects.minor_globe_of_invulnerability, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protect_magic, player);
                    break;

                case CheckType.CanHitAttacker:
                    await ovr024.calc_affect_effect(Classes.Affects.bless, player);
                    await ovr024.calc_affect_effect(Classes.Affects.cursed, player);
                    await ovr024.calc_affect_effect(Classes.Affects.blinded, player);
                    await ovr024.calc_affect_effect(Classes.Affects.bestow_curse, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prayer, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_flame_tongue, player);
                    await ovr024.calc_affect_effect(Classes.Affects.gnome_vs_goblin_kobold, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dwarf_vs_orc_goblin, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_dragon_slayer, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_frost_brand, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_undead_slayer, player);
                    break;
                ...
                case CheckType.Type_11:
                    await ovr024.calc_affect_effect(Classes.Affects.blinded, player);
                    await ovr024.calc_affect_effect(Classes.Affects.shield, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protection_from_evil, player);
                    await ovr024.calc_affect_effect(Classes.Affects.protection_from_good, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_from_evil_10_radius, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_from_good_10_radius, player);
                    await ovr024.calc_affect_effect(Classes.Affects.stinking_cloud, player);
                    await ovr024.calc_affect_effect(Classes.Affects.faerie_fire, player);
                    break;
                ...
                case CheckType.SavingThrow:
                    ...
                    await ovr024.calc_affect_effect(Classes.Affects.prot_paralysis_poison, player);
                    await ovr024.calc_affect_effect(Classes.Affects.prot_sleep_charm_paralysis_poison, player);
                    await ovr024.calc_affect_effect(Classes.Affects.con_saving_bonus, player);
                    await ovr024.calc_affect_effect(Classes.Affects.hot_fire_shield, player);
                    await ovr024.calc_affect_effect(Classes.Affects.cold_fire_shield, player);
                    break;

                case CheckType.Death:
                    await ovr024.calc_affect_effect(Classes.Affects.fight_unconscious, player);
                    await ovr024.calc_affect_effect(Classes.Affects.troll_fire_or_acid, player);
                    await ovr024.calc_affect_effect(Classes.Affects.weap_dragon_slayer, player);
                    break;

                case CheckType.Type_14:
                    await ovr024.calc_affect_effect(Classes.Affects.petrifying_gaze, player);
                    await ovr024.calc_affect_effect(Classes.Affects.breath_elec, player);
                    await ovr024.calc_affect_effect(Classes.Affects.ankheg_ranged_acid_attack, player);
                    await ovr024.calc_affect_effect(Classes.Affects.spit_acid, player);
                    await ovr024.calc_affect_effect(Classes.Affects.beholder_eyestalk, player);
                    await ovr024.calc_affect_effect(Classes.Affects.breath_acid, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dracolich_paralytic_gaze, player);
                    await ovr024.calc_affect_effect(Classes.Affects.breath_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.cast_breath_fire, player);
                    await ovr024.calc_affect_effect(Classes.Affects.cast_throw_lightening, player);
                    await ovr024.calc_affect_effect(Classes.Affects.affect_8b, player);
                    break;

                case CheckType.Type_15:
                    await ovr024.calc_affect_effect(Classes.Affects.silence_15_radius, player);
                    await ovr024.calc_affect_effect(Classes.Affects.stinking_cloud, player);
                    await ovr024.calc_affect_effect(Classes.Affects.charm_person, player);
                    await ovr024.calc_affect_effect(Classes.Affects.reduce, player);
                    await ovr024.calc_affect_effect(Classes.Affects.berserk, player);
                    break;
                ...
                case CheckType.CanHitTarget:
                    ...
                    await ovr024.calc_affect_effect(Classes.Affects.invisible, player);
                    await ovr024.calc_affect_effect(Classes.Affects.blink, player);
                    await ovr024.calc_affect_effect(Classes.Affects.giant_vs_dwarf_gnome, player);
                    await ovr024.calc_affect_effect(Classes.Affects.gnoll_bugbear_vs_gnome, player);
                    await ovr024.calc_affect_effect(Classes.Affects.displace, player);
                    await ovr024.calc_affect_effect(Classes.Affects.dispel_evil, player);
                    break;
                ...
                case CheckType.Movement:
                    await ovr024.calc_affect_effect(Classes.Affects.haste, player);
                    await ovr024.calc_affect_effect(Classes.Affects.slow, player);
                    await ovr024.calc_affect_effect(Classes.Affects.clear_movement, player);
                    break;

                case CheckType.BattleRound:
                    await ovr024.calc_affect_effect(Classes.Affects.regen_3_hp, player);
                    await ovr024.calc_affect_effect(Classes.Affects.spiritual_hammer, player);
                    await ovr024.calc_affect_effect(Classes.Affects.camouflage, player);
                    await ovr024.calc_affect_effect(Classes.Affects.item_invisibility, player);
                    await ovr024.calc_affect_effect(Classes.Affects.charm_person, player);
                    break;
                ...
                case CheckType.Confusion:
                    await ovr024.calc_affect_effect(Classes.Affects.confuse, player);
                    break;

                case CheckType.BattleSetup2:
                    await ovr024.calc_affect_effect(Classes.Affects.add_invisibility, player);
                    break;

                case CheckType.Type_23:
                    await ovr024.calc_affect_effect(Classes.Affects.affect_4a, player);
                    break;
                default:
                    return false;
            }
